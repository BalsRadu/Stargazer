  ...
  login: (req, res) => {
    model.findOne({ email: req.body.email }, (err, user) => {
      ...
      user.comparePassword(req.body.password, (err, isMatch) => {
        if (err) throw err;
        if (isMatch) {
          let jwtToken = jwt.sign({ id: user._id }, config.secret, {
            expiresIn: 86400,
          });
  ...
  editprofile: (req, res) => {
    ...
    model
      ...
      .catch((err) => {
        res.send({
          success: false,
          msg: "Something went wrong durring the updating proccess",
        });
      });
  },
  follow: (req, res) => {
    let user_id = jwt.decode(req.body.auth_token).id;
    let user_to_follow = req.body.user_id;
    model
      .findByIdAndUpdate(user_to_follow, {
      ...
      .then((oldUser) => {
        model
          ...
          .then((oldUser) => {
            res.send({ success: true, msg: "User successfully followed" });
          });
      });
  },
  unfollow: (req, res) => {
    ...
    model
      ...
      .then((oldUser) => {
        model
          ...
          .then((oldUser) => {
            res.send({ success: true, msg: "User successfully unfollowed" });
          });
      });
  },
};
